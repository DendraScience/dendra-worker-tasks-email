...
module.exports = {
  ...
  async execute(m, {
    ...
    let publishCount = 0;
    let flaggedCount = 0;
    logger.info('Searching mailbox', {
      query
    });

    try {
      foundUids = await imapClient.search(query, {
        uid: true
      });
    ...
    for (const uid of foundUids) {
      ...
      try {
        message = await imapClient.fetchOne(uid, {
      ...
      if (message) {
        ...
        try {
          ...
          if (bodyParts) {
            if (Array.isArray(decodeParts)) decodeParts.forEach(({
              key,
              type
            }) => {
              let value = bodyParts.get(key);
              if (type === 'string') value = value.toString();else if (type === 'base64') value = Buffer.from(value.toString(), 'base64');
      ...
      if (guid) {
        logger.info('Flagging message', {
          uid
        });

        try {
          await imapClient.messageFlagsAdd(uid, ['\\Flagged'], {
            uid: true
          });
          flaggedCount++;
  ...
  assign(m, res, {
    ...
    logger.info(`Found (${res.foundCount}) messages(s), selected (${res.selectCount}), published (${res.publishCount}), flagged (${res.flaggedCount})`);
    m.totalFlaggedCount += res.flaggedCount;
    m.healthCheckTs = new Date().getTime();
    m.sourceIndex++; // Check mail on the next run if we found more than we processed
